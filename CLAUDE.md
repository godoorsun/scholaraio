...
- **自由分析中的文献呈现**：在自由探索、对比分析、调研讨论这类回答里，如果只涉及少量论文，先定义一个简短的编号参考表（如 `[1]`、`[2]`、`[3]`，每个编号只映射一次到论文标题/链接），正文随后只使用编号引用，不要在正文里直接插入很长的绝对路径。只有用户明确要求文件路径时，才在正文中直接给出路径。若涉及论文较多，正文仍优先使用编号，参考映射表或参考文献列表放在文前或文后集中呈现。
- **入库安全**：普通 inbox 入库默认不要为了提速而临时拆成“本地并行 MinerU 预转换”再二次入库，除非用户明确要求特殊流程。MinerU 生成的图片资源（`*_images` / `*_mineru_images`）必须随正文一起入库，并统一规范成论文目录下的 `images/` 文件夹。
